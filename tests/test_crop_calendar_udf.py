...
class TestCropCalendarUdf(unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        # load inputs
        with open('tests/resources/Field_BE.geojson') as f:
        ...
        self.context={
            ...
            'path_harvest_model': 'Pilot1/Tests/Cropcalendars/Model/model_update1.0_iteration24.h5', 
            'shub': None, # TODO: don't forget to set this 
    ...
    def test_01_CropCalendarCalculation(self):
        ...
        with open(Path(str(self.tmp.name),"result.json"),"w") as f:
            json.dump(res, f)
        
        
    def test_02_CropsarCurveIsSame(self):

        with open("tests/resources/input_df_cropsar.json") as f: 
            ref= self.processTS(json.load(f))

        with open(Path(str(self.tmp.name),"input_df_cropsar.json")) as f:
    ...
    def test_03_InputToModelIsSame(self):
        ...
        with open("tests/resources/input_df_model.json") as f: 
            ref= self.processTS(json.load(f))

        with open(Path(str(self.tmp.name),"input_df_model.json")) as f:
            res=self.processTS(json.load(f))
    ...
    def test_04_ResultIsSame(self):
        ...
        with open(Path(str(self.tmp.name),"result.json")) as f:
            res=json.load(f)
        with open("tests/resources/Test_fields_after_RO_selection_and_orbit_direction_SHUB_gjson_load_udf.json") as f:
            ref=json.load(f)
