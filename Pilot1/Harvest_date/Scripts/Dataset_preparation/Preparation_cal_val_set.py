...
ro_s = ['ro110','ro161'] # the ro's for which you want to extract the data from
Test_nr = r'Test12'
...
shp_flax = gpd.read_file(r"S:\eshape\Pilot 1\results\Harvest_date\Training_Val_selection\2018_Flax_fields_selected.shp")
ids_flax_selected = shp_flax.id.to_list()
shp_WIG_2018 = gpd.read_file(r"S:\eshape\Pilot 1\results\Harvest_date\Training_Val_selection\2018_WIG_fields_selected.shp")
ids_WIG_2018 = shp_WIG_2018.id.to_list()
shp_WIG_2019 = gpd.read_file(r"S:\eshape\Pilot 1\results\Harvest_date\Training_Val_selection\2019_WIG_fields_selected.shp")
...
ids_remove = ['AWpnndUzg8l355Cx1_aC', 'AWmwd4Xj8cDLxG4hgS38', 'AWztFmWjPkIpp4CeUG4F',
              'AWpe5oDKg8l355Cx0x58']  # ids to remove based on results from Test1
...
# a)Filter the coherence df so only the VV coherence is used
        for ro_interest in ro_s:
            ...
            #rescale the VHVV ratio to [-1,1]
            df_VHVV_ratio_fields_selected = 2*(df_VHVV_ratio_fields_selected-VH_VV_range[0])/(VH_VV_range[1]-VH_VV_range[0])-1

            df_cropsar_fields_selected = df_cropsar_combined.loc[:,df_cropsar_combined.columns.isin(ids_cal_val)]
            # rescale the fAPAR to values between -1 and 1
            df_cropsar_fields_selected = 2*(df_cropsar_fields_selected-fAPAR_range[0])/(fAPAR_range[1]-fAPAR_range[0])-1

            df_cropsar_fields_selected = df_cropsar_fields_selected.reindex(df_VHVV_ratio_fields_selected.index, method= 'nearest') # set that the fAPAR df to the same temporal resolution as for coherence and S1
            df_slope_fAPAR_fields_selected = df_slope_fAPAR_combined.loc[:,df_slope_fAPAR_combined.columns.isin(ids_cal_val)]
            df_slope_fAPAR_fields_selected = df_slope_fAPAR_fields_selected.reindex(df_VHVV_ratio_fields_selected.index, method= 'nearest')
            ...
            ### calculate the difference in days with the harvest date
                ...
                df_harvest_values_field_cropsar = df_cropsar_fields_selected.loc[:,df_cropsar_fields_selected.columns.isin([id_cal_val])][df_diff_harvest_cropsar.Date <= pd.to_timedelta(0)].iloc[-values_before_harvest:] ### Extraction of fAPAR for two closest dates before harvest
                df_harvest_values_field_cropsar = df_harvest_values_field_cropsar.append(df_cropsar_fields_selected.loc[:,df_cropsar_fields_selected.columns.isin([id_cal_val])][df_diff_harvest_cropsar.Date > pd.to_timedelta(0)].iloc[0:values_after_harvest]) ### Extraction of fAPAR for two closest dates after harvest
                ...
                df_harvest_values_field_slope_fAPAR = df_slope_fAPAR_fields_selected.loc[:,df_slope_fAPAR_fields_selected.columns.isin([id_cal_val])][df_diff_harvest_slope_fAPAR.Date <= pd.to_timedelta(0)].iloc[-values_before_harvest:]  ### Extraction of fAPAR for two closest dates before harvest
                df_harvest_values_field_slope_fAPAR = df_harvest_values_field_slope_fAPAR.append(df_slope_fAPAR_fields_selected.loc[:,df_slope_fAPAR_fields_selected.columns.isin([id_cal_val])][df_diff_harvest_slope_fAPAR.Date > pd.to_timedelta(0)].iloc[0:values_after_harvest])  ### Extraction of fAPAR for two closest dates after harvest
                ...
                df_harvest_values_field_vhvv = df_VHVV_ratio_fields_selected.loc[:,df_VHVV_ratio_fields_selected.columns.isin(['VH_VV_'+id_cal_val])][df_diff_harvest_vhvv.Date <= pd.to_timedelta(0)].iloc[-values_before_harvest:] ### Extraction of VHVV for two closest dates before harvest

                df_harvest_values_field_vhvv = df_harvest_values_field_vhvv.append(df_VHVV_ratio_fields_selected.loc[:,df_VHVV_ratio_fields_selected.columns.isin(['VH_VV_'+id_cal_val])][df_diff_harvest_vhvv.Date > pd.to_timedelta(0)].iloc[0:values_after_harvest]) ### Extraction of VHVV for two closest dates after harvest
                ...
                df_harvest_data_extraction.append(pd.concat([pd.DataFrame(df_harvest_values_field_cropsar.T.values, index = [id_cal_val], columns = (['fAPAR_{}'.format(n) for n in range(1,values_before_harvest+values_after_harvest+1)])),
                                 pd.DataFrame(df_harvest_values_field_vhvv.T.values, index = [id_cal_val], columns = (['ro110_VHVV_{}'.format(n) for n in range(1,values_before_harvest+values_after_harvest+1)])),  pd.DataFrame(df_harvest_values_field_slope_fAPAR.T.values, index = [id_cal_val], columns = (['slope_fAPAR_{}'.format(n) for n in range(1,values_before_harvest+values_after_harvest+1)]))], axis=1)) #pd.DataFrame(df_harvest_values_field_coherence.T.values, index = [id_cal_val], columns = (['ro110_coh_{}'.format(n) for n in range(1,5)])),
                ...
                random_sample_excl = []
                date_range_after_harvest = pd.date_range(start = (df_VHVV_ratio_fields_selected.loc[:, df_VHVV_ratio_fields_selected.columns.isin(['VH_VV_' + id_cal_val])][df_diff_harvest_vhvv.Date > pd.to_timedelta(0)].dropna().iloc[values_after_harvest+1:].index[0].strftime('%Y-%m-%d')), end = (df_VHVV_ratio_fields_selected.loc[:, df_VHVV_ratio_fields_selected.columns.isin(['VH_VV_' + id_cal_val])][df_diff_harvest_vhvv.Date > pd.to_timedelta(0)].dropna().iloc[-1:].index[0].strftime('%Y-%m-%d')), freq='6D')
                random_sample = [0]
                ## pick random date out of date range
                while random_sample and counter_after_harvest_select <3:
                    try:
                        random_sample = choice([i for i in range(1, len(date_range_after_harvest) - (values_after_harvest+values_before_harvest-1)) if i not in random_sample_excl]) # see that the boundary of the range are excluded for period selection
                    except:
                        random_sample = [] # no sampling possibilities left
                    if not random_sample:
                        continue
                    df_no_harvest_values_field_vhvv = df_VHVV_ratio_fields_selected.loc[:,df_VHVV_ratio_fields_selected.columns.isin(['VH_VV_'+id_cal_val])][df_VHVV_ratio_fields_selected.index >= date_range_after_harvest[random_sample-1]].iloc[0:values_after_harvest+values_before_harvest] # take all the harvest values around the selected date
                    ...
                    #     continue
                    df_no_harvest_values_field_cropsar = df_cropsar_fields_selected.loc[:,df_cropsar_fields_selected.columns.isin([id_cal_val])][df_cropsar_fields_selected.index >= date_range_after_harvest[random_sample-1]].iloc[0:values_before_harvest+values_after_harvest]

                    df_no_harvest_values_field_slope_fAPAR = df_slope_fAPAR_fields_selected.loc[:,df_slope_fAPAR_fields_selected.columns.isin([id_cal_val])][df_slope_fAPAR_fields_selected.index >= date_range_after_harvest[random_sample-1]].iloc[0:values_before_harvest+values_after_harvest]
                    df_non_harvest_data_extraction.append(pd.concat([pd.DataFrame(df_no_harvest_values_field_cropsar.T.values, index=[id_cal_val+'_after_{}'.format(str(counter_after_harvest_select))], columns=(['fAPAR_{}'.format(n) for n in range(1, values_after_harvest+values_before_harvest+1)])),
                                                                 pd.DataFrame(df_no_harvest_values_field_vhvv.T.values,index=[id_cal_val+'_after_{}'.format(str(counter_after_harvest_select))], columns=(['ro110_VHVV_{}'.format(n) for n in range(1, values_after_harvest+values_before_harvest+1)])),
                                                                     pd.DataFrame(df_no_harvest_values_field_slope_fAPAR.T.values, index=[id_cal_val+'_after_{}'.format(str(counter_after_harvest_select))], columns=(['slope_fAPAR_{}'.format(n) for n in range(1, values_after_harvest+values_before_harvest+1)]))], axis=1)) #pd.DataFrame(df_no_harvest_values_field_coherence.T.values,index=[id_cal_val+'_after_{}'.format(str(counter_after_harvest_select))], columns=(['ro110_coh_{}'.format(n) for n in range(1, 5)])),
                    random_sample_excl.extend([random_sample-4,random_sample-3, random_sample -2, random_sample-1, random_sample, random_sample+1, random_sample+2, random_sample +3, random_sample+4])
                ...
                random_sample_excl = []
                date_range_before_harvest = pd.date_range(start=(df_VHVV_ratio_fields_selected.loc[:, df_VHVV_ratio_fields_selected.columns.isin(['VH_VV_' + id_cal_val])][df_diff_harvest_vhvv.Date <= pd.to_timedelta(0)].dropna().iloc[:-values_before_harvest].index[0].strftime('%Y-%m-%d')),
                                                          end=(df_VHVV_ratio_fields_selected.loc[:, df_VHVV_ratio_fields_selected.columns.isin(['VH_VV_' + id_cal_val])][df_diff_harvest_vhvv.Date <= pd.to_timedelta(0)].dropna().iloc[:-values_before_harvest].index[-1].strftime('%Y-%m-%d')),
                                                         freq='6D')
                if df_VHVV_ratio_fields_selected.loc[:, df_VHVV_ratio_fields_selected.columns.isin(['VH_VV_' + id_cal_val])][df_diff_harvest_vhvv.Date <= pd.to_timedelta(0)].dropna().iloc[:-values_before_harvest].index[0].month <5:
                    date_range_before_harvest = pd.date_range(start='{}-05-01'.format(str(df_harvest_date.year.values[0])),
                                                                           ...
                                                                           0)].dropna().iloc[:-values_before_harvest].index[-1].strftime('%Y-%m-%d')),
                                                              freq='6D')
                random_sample = [0]
                while random_sample and counter_before_harvest_select < 3:
                    try:
                        random_sample = choice([i for i in range(1, len(date_range_before_harvest) - (values_before_harvest+ values_after_harvest-1)) if i not in random_sample_excl])  # see that the boundary of the range are excluded for period selection
                    except:
                        random_sample = [] # no sampling possibilities left
                    if not random_sample:
                        continue
                    df_no_harvest_values_field_vhvv = df_VHVV_ratio_fields_selected.loc[:, df_VHVV_ratio_fields_selected.columns.isin(['VH_VV_' + id_cal_val])][df_VHVV_ratio_fields_selected.index >= date_range_before_harvest[random_sample - 1]].iloc[0:values_after_harvest+values_before_harvest]  # take all the harvest values around the selected date
                    if df_no_harvest_values_field_vhvv.isnull().values.any() or df_no_harvest_values_field_vhvv.isna().values.any():
                        random_sample_excl.extend([random_sample-4,random_sample - 3, random_sample - 2, random_sample - 1, random_sample, random_sample + 1,random_sample + 2, random_sample + 3, random_sample+4])
                        continue
                    ...
                    #     random_sample_excl.extend([random_sample - 3, random_sample - 2, random_sample - 1, random_sample, random_sample + 1,random_sample + 2, random_sample + 3])
                    #     continue
                    df_no_harvest_values_field_cropsar = df_cropsar_fields_selected.loc[:, df_cropsar_fields_selected.columns.isin([id_cal_val])][df_cropsar_fields_selected.index >= date_range_before_harvest[random_sample - 1]].iloc[0:values_before_harvest+values_after_harvest]
                    df_no_harvest_values_field_slope_fAPAR = df_slope_fAPAR_fields_selected.loc[:,df_slope_fAPAR_fields_selected.columns.isin([id_cal_val])][df_slope_fAPAR_fields_selected.index >= date_range_before_harvest[random_sample-1]].iloc[0:values_before_harvest+values_after_harvest]
                    df_non_harvest_data_extraction.append(pd.concat([pd.DataFrame(df_no_harvest_values_field_cropsar.T.values,index=[id_cal_val + '_before_{}'.format(str(counter_before_harvest_select))], columns=(['fAPAR_{}'.format(n) for n in range(1, values_before_harvest+values_after_harvest+1)])),
                                                                     pd.DataFrame(df_no_harvest_values_field_vhvv.T.values, index=[id_cal_val + '_before_{}'.format(str(counter_before_harvest_select))], columns=(['ro110_VHVV_{}'.format(n) for n in range(1, values_before_harvest+values_after_harvest+1)])),
                                                                     pd.DataFrame(df_no_harvest_values_field_slope_fAPAR.T.values, index=[id_cal_val+'_before_{}'.format(str(counter_before_harvest_select))], columns=(['slope_fAPAR_{}'.format(n) for n in range(1, values_before_harvest+values_after_harvest+1)]))], axis=1)) #pd.DataFrame(df_no_harvest_values_field_coherence.T.values,index=[id_cal_val + '_before_{}'.format(str(counter_before_harvest_select))], columns=(['ro110_coh_{}'.format(n) for n in range(1, 5)])),
                    random_sample_excl.extend([random_sample-4, random_sample - 3, random_sample - 2, random_sample - 1, random_sample, random_sample + 1,random_sample + 2, random_sample + 3, random_sample +4])
            ...
            if 'calibration' in dataset:
                df_calibration.append(pd.concat([df_harvest_data_extraction, df_non_harvest_data_extraction]))
            else:
                df_validation.append(pd.concat([df_harvest_data_extraction, df_non_harvest_data_extraction]))
    #### save calibration file for the iteration
    ...
    # save validation file for the iteration
