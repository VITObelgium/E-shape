...
for ro_interest in ro_s:
    ...
    datasets_dict = {'Flax':r"S:\eshape\Pilot 1\results\Harvest_date\Training_Val_selection\2018_Flax_fields_selected.shp",'2018_WIG':r"S:\eshape\Pilot 1\results\Harvest_date\Training_Val_selection\2018_WIG_fields_selected.shp",
                         '2019_WIG':r"S:\eshape\Pilot 1\results\Harvest_date\Training_Val_selection\2019_WIG_fields_selected.shp",'TAP':r"S:\eshape\Pilot 1\data\TAP_monitoring_experiment\2019_TAP_monitoring_experiment.shp"}
    if not Flax:
        datasets_dict.pop('Flax')
    if not WIG_2018:
        datasets_dict.pop('2018_WIG')
    if not WIG_2019:
        datasets_dict.pop('2019_WIG')
    if not TAP:
        datasets_dict.pop('TAP')
    ...
    def compile_harvest_model_data_propagating_window(df_cropsar_combined,df_VHVV_combined,fields_dataset_compiling, df_harvest_dates):
        window_values = 5 # 4 coverage are extracted within the window
        moving_window_steps = np.arange(0, df_cropsar_combined.shape[0] - window_values-1)
        ...
        for id in fields_dataset_compiling:
            ...
            for p in range(len(moving_window_steps)):
                ...
                if df_cropsar_field_window.isnull().values.any() or df_cropsar_field_window.isnull().values.any():
                    continue
                prediction_date_window =  pd.DataFrame(df_cropsar_field_window.index[0] + timedelta(window_width/2), index = [id+'_{}_{}'.format(ro_interest,str(p))], columns=(['prediction_date_window']))#the center data of the window which can is in fact the harvest prediction date if the model returns 1
                harvest_date_field = pd.DataFrame([df_harvest_dates.loc[df_harvest_dates.id == id]['Harvest_date'].values[0]] , index = [id+'_{}_{}'.format(ro_interest,str(p))], columns=['Harvest_date'])
                df_cropsar_field_window = pd.DataFrame(df_cropsar_field_window.T.values, index = [id+'_{}_{}'.format(ro_interest,str(p))], columns= (['fAPAR_{}'.format(n) for n in range(1,window_values+1)]))
                ...
                df_dates_window = pd.DataFrame(df_dates_window.T.values, index = [id+'_{}_{}'.format(ro_interest,str(p))], columns= (['Date_{}'.format(n) for n in range(1,window_values+1)]))
                ...
                df_diff_harvest_window = pd.DataFrame(df_diff_harvest_window.T.values, index = [id+'_{}_{}'.format(ro_interest,str(p))], columns= (['Diff_harvest_{}'.format(n) for n in range(1,window_values+1)]))
                df_VHVV_field_window = pd.DataFrame(df_VHVV_field_window.T.values, index = [id+'_{}_{}'.format(ro_interest,str(p))], columns= (['ro110_161_VHVV_{}'.format(n) for n in range(1,window_values+1)]))
    ...
    df_harvest_model.append(compile_harvest_model_data_propagating_window(df_cropsar_combined,df_VHVV_combined,fields_dataset_compiling, df_harvest_dates))
df_harvest_model = pd.concat(df_harvest_model)
if metrics_exclude:
    for s in range(len(metrics_exclude)):
        df_harvest_model.drop([col for col in df_harvest_model if metrics_exclude[s] in col],
                                        axis=1, inplace=True)
#df_harvest_model.to_csv(r'S:\eshape\Pilot 1\data\model_harvest_detection\model_Kasper\validation\6_daily_window_data\df_harvest_model_6daily_TAP_only_ro110_ro161_30_daily_window_only_fAPAR.csv')
