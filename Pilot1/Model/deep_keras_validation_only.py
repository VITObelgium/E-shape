...
df_validation=pd.read_csv(r"S:\eshape\Pilot 1\data\model_harvest_detection\model_Mehrdad\validation\Validation_data_eshape_2019.csv")
df_calibration=pd.read_csv(r"S:\eshape\Pilot 1\data\model_Mehrdad\calibration\df_calibration.csv")

x_train = df_calibration.iloc[0:470,0:15]
y_train = df_calibration.iloc[0:470,15:16] #### the collum we want to predict (target)
weight_train=np.ones(y_train.shape)
weight_train[y_train==1]=10

x_test = df_validation.iloc[:,1:16]
y_test = df_validation.iloc[:,16:17]
...
logger = CSVLogger('fit.log', separator=',', append=False)
...
checkpoint=ModelCheckpoint('model.h5', monitor='val_loss', verbose=0, save_best_only=True, save_weights_only=False, mode='auto', period=1) #Save the weight of the model so that they can be used later
...
df_validation.to_csv(r"S:\eshape\Pilot 1\data\model_harvest_detection\model_Mehrdad\validation\Validation_data_eshape_2019_predictions.csv")
pdb.set_trace()
